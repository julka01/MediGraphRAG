...
describe('MainLayout', () => {
  ...
  it('renders both panels in split layout', () => {
    render(<MainLayout layout="split" graphPanel={graphPanel} chatPanel={chatPanel} />);
    expect(screen.getByTestId('graph')).toBeInTheDocument();
    expect(screen.getByTestId('chat')).toBeInTheDocument();
  });

  it('renders only graph in graph-only layout', () => {
    render(<MainLayout layout="graph-only" graphPanel={graphPanel} chatPanel={chatPanel} />);
    expect(screen.getByTestId('graph')).toBeInTheDocument();
    expect(screen.queryByTestId('chat')).not.toBeInTheDocument();
  });

  it('renders only chat in chat-only layout', () => {
    render(<MainLayout layout="chat-only" graphPanel={graphPanel} chatPanel={chatPanel} />);
