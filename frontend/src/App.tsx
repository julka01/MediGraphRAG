  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-base-100 text-base-content">
      {state.sidebarCollapsed && (
        <button className="absolute top-2 left-1 z-30 btn btn-ghost btn-xs" onClick={() => dispatch({ type: 'TOGGLE_SIDEBAR' })} title="Expand sidebar">›</button>
      )}

      <Sidebar kgModelHook={kgModelHook} ragModelHook={ragModelHook} onNeo4jOpen={() => setNeo4jOpen(true)} onProgressStart={() => setProgressActive(true)} onProgressStop={() => setProgressActive(false)} />
