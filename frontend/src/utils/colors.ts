...
function getNodePalette(): readonly string[] {
  return NODE_PALETTE_FALLBACKS.map((fallback, i) => readCSSColor(`--color-graph-node-${i}`, fallback));
}

function getEdgePalette(): readonly string[] {
  return EDGE_PALETTE_FALLBACKS.map((fallback, i) => readCSSColor(`--color-graph-edge-${i}`, fallback));
}
